"""Per-process sliding-window rate limiter (shared kernel).

A business-neutral abuse-damping primitive. It lives in ``core`` so any domain
package can reuse it without a lateral import; the notify relay and the V1 web
execution gate both do.

The backend has no shared rate-limit substrate (no Redis; PG would put a hot
write on every call), so this is plain **per-worker process memory**. Under
gunicorn ``-w N`` the effective ceiling is therefore ≈ N × the configured limit
— acceptable for abuse damping, NOT precise quota accounting. Any caller that
needs a global ceiling must build on a shared store instead.
...
# Hard cap on tracked keys. Per-key spaces are tiny in practice (one key per
# token / user / caller IP); this only guards against a deliberate flood of
...
class SlidingWindowLimiter:
    ...
    def allow(self, key: str, limit: int, window: float) -> tuple[bool, float]:
        ...
        leaves the window (for a 429 ``Retry-After`` header / retry hint)."""
        now = time.monotonic()
        with self._lock:
    ...
    def peek(self, key: str, limit: int, window: float) -> tuple[bool, float]:
        ...
        expensive work, while the actual charge happens only when the request
    ...
    def _sweep(self, now: float, window: float) -> None:
        ...
        # the window (dead weight); any still-hot key survives because its
        # newest hit is recent.
