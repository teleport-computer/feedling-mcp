"""Daily trace write-rate counters (T138 块 0：先量再定).

Revision ID: 0095_trace_write_stats
Revises: 0094_chat_daily_rollup
...
它保存的是"截断之后剩下的"，不是"发生过的"。拿它统计会把被截掉的量当成不存在，
再把峰值当成均值 —— 而容量是按峰值定的。

所以块 0 必须先建一个**能活过重启的持久日计数器**（就是本表），然后连续量 7 天
取峰值。这也是为什么它带一个 7 天墙钟窗、且是整件事的长杆。
...
revision = "0095_trace_write_stats"
down_revision = "0094_chat_daily_rollup"
branch_labels = None
depends_on = None
