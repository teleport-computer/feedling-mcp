...
Revision ID: 0104_distill_artifact_ledger
Revises: 0103_v2_wake_followup_marker

The attempt table starts empty by design.  Older distillation jobs do not
contain per-artifact attempt boundaries, so reconstructing rows from their
final summaries would turn "not measured" into invented evidence.
"""

from alembic import op


revision = "0104_distill_artifact_ledger"
down_revision = "0103_v2_wake_followup_marker"
branch_labels = None
depends_on = None
...
_DOWN = """
DROP TABLE IF EXISTS distillation_rollup_watermark;
DROP TABLE IF EXISTS distillation_artifact_daily_rollup;
DROP TABLE IF EXISTS distillation_artifact_attempts;
ALTER TABLE genesis_import_jobs DROP COLUMN IF EXISTS failed_phase;
"""
