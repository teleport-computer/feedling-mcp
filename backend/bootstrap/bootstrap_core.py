...
def bootstrap_payload(store: UserStore) -> dict:
    ...
    instructions = (
        ...
        "AGENT_HTTP_URL 或 AGENT_CLI_CMD。若 HTTP agent 与 consumer 同机、能执行本地命令且继承 "
        "FEEDLING_HOME，显式设置 FEEDLING_AGENT_HTTP_LOCAL_IO_CLI=true，并把 "
        "FEEDLING_AGENT_HTTP_LOCAL_FILE_ROOTS 设为 Agent 的精确工作区目录，让它能通过 "
        "io_cli 交付文件和 .io.html Canvas；不要授权主目录或文件系统根目录，普通或远程 "
        "HTTP 模型服务必须保持 false。"
        "consumer 持续轮询 /v1/chat/poll，"
