...
def _memory_add_action(
    ...
    *,
    ...
    if card_guard.guard_enabled() and _memory_tombstone_reason(
        summary, _memory_content_from_action(raw, summary)
    ):
...
def _memory_upgrade_action(store: UserStore, api_key: str | None, action: dict) -> tuple[dict, list[dict], int]:
    ...
    if card_guard.guard_enabled() and _memory_tombstone_reason(
        inner.get("summary"), inner.get("content")
    ):
