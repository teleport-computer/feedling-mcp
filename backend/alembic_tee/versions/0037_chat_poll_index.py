"""Add the bounded resident-poll index to the TEE primary schema.

Revision ID: 0037_chat_poll_index
Revises: 0036_lane_rollup_access_paths

The durable chat change-control tables remain RDS-only during the shadow phase.
This index is different: ``chat_messages`` is a shared runtime table, so the
promoted TEE database must preserve the same bounded poll query plan.
"""

from alembic import op


revision = "0037_chat_poll_index"
down_revision = "0036_lane_rollup_access_paths"
branch_labels = None
depends_on = None
...
SET value = convert_to(
  jsonb_set(convert_from(value, 'UTF8')::jsonb, '{tee_heads}',
            '["0037_chat_poll_index"]'::jsonb)::text,
  'UTF8'
)
