...
1. CIPHERTEXT lane（7 张：chat_message_archive / model_api_credentials /
   ...
   v2_trajectory_reviews / v2_workspace_entries）的信封列在 TEE 侧存明文。本 revision 创建
   时，RDS 0043 的 ck_v2_trajectory_envelope 仅允许 body_ct/K_enclave 等密文键，因而
   没有复制；后续 RDS 0072 已放宽为同时允许 TEE worker 产出的精确四键明文 body 形状，
   所以该缺口现为 T149/A-17 约束债，而非永久例外。derive_tee_ddl.py 本身不导出任何
   CHECK，T149 复核最终 migration chain 后共发现 10 张表 17 条同根因 CHECK 债；均须在
   目标 RDS 先包含 0072 且 TEE 实库行预检通过后，以后续 migration 补齐。
...
-- T149: known parity debt.  The derivation omitted five RDS CHECKs; the
-- envelope predicate's current source-of-truth is the two-branch RDS 0072
-- definition.  The composite stream FK is separately omitted by §2 above and
-- must be re-evaluated for TEE-primary rather than copied blindly.
...
-- T149/B: job_id→agent_jobs is the cross-table FK intentionally omitted by §2;
-- its "revisit after write paths land" trigger is now satisfied by TEE-primary.
...
-- T149/A: derive_tee_ddl omitted the RDS body_key and attempt_count CHECKs.
...
-- T149/A: derive_tee_ddl omitted the RDS generation/count CHECKs.
...
-- T149/A: derive_tee_ddl omitted the RDS day-format CHECK.
...
-- T149/A: derive_tee_ddl omitted the RDS cohort-date and period CHECKs.
...
-- T149/A: derive_tee_ddl omitted the RDS claim_state CHECK.
...
-- T149/A: derive_tee_ddl omitted the RDS single-row id=1 CHECK.
...
-- T149/A: derive_tee_ddl omitted the RDS desired-runtime CHECK.
...
-- T149/A: derive_tee_ddl omitted the RDS capacity CHECK.
