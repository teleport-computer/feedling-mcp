...
def _wake_discarded_draft_prompt_data(draft: dict) -> dict:
    ...
    # The cap applies to what the provider actually receives, not to Python's
    # pre-encoding character count.  Newlines, quotes and backslashes expand
    # under JSON escaping, so a direct ``raw_text[:remaining]`` can still exceed
    # the cap.  Binary-search the longest prefix in rendered space (same shape
    # as tool_loop._rejected_tool_args) and make overflow unreachable.
    best = ""
    low = 0
    high = len(raw_text)
    while low <= high:
...
def _wake_action_context_str(
    ...
    ordinary grounding observations are then added whole until the remaining
    budget is exhausted.
    """
    ...
    except Exception as exc:  # noqa: BLE001 — stale context must never kill wake
        # Weak wakes are intentionally fail-safe.  A malformed/stale breadcrumb
        # may be omitted, but it must not make heartbeat/manual/screen wake fail
        # forever before the provider call and therefore never reach applied
        # cleanup.
        log.warning(
...
async def _run_wake(
    ...
    try:
        ...
        if lane in _WAKE_DISCARDED_DRAFT_LANES:
            ...
            except Exception as exc:  # noqa: BLE001 — stale hint is optional context
                log.warning(
            ...
            if seq_native:
                ...
                if status == "applied":
                    ...
                    if (
                        ...
                        except Exception as exc:  # noqa: BLE001 — reply is applied
                            cleared = False
                            log.warning(
                ...
                if (
                    ...
                    # only a bounded encrypted draft so the next weak wake knows
                    # this exact wording was never shown to the user.  The small
                    # post-discard/pre-log crash window intentionally degrades to
                    # the pre-T310 behavior; it never retries or publishes here.
                    try:
                    ...
                    except Exception as exc:  # noqa: BLE001 — accepted fail-safe gap
                        stored_draft = None
                        log.warning(
