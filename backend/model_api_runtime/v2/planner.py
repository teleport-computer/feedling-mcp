...
Global Constraints "no behavior tiering"）。`worker.py` 与 D4 token-per-turn 回滚门自
PR C9b 起都不再 import 本模块；D4 直接驱动生产 `tool_loop.run_tool_loop`。本模块暂留给
旧 planner 的兼容性单测（`tests/test_v2_planner*.py`）。
`tests/test_v2_no_dispatch_tiering.py` 锁定的真正不变量是生产代码和回滚门都不调用这些
函数，而不是"这个模块不存在"。
