...
def _leaf_hint_ranges(
    ...
    *,
    ...
    sealed_leaves: list[dict] = []
    local_legacy_hits: list[str] = []
    for leaf in picked:
        env = leaf.get("summary_envelope")
        shape = core_envelope.classify_envelope_shape(env)
        if shape in ("plaintext_text", "plaintext_binary"):
            try:
                text = core_envelope.read_plaintext_envelope_body(
                    env, owner_user_id=owner_user_id).decode(
                        "utf-8", errors="replace")
            except ValueError:
                continue
            if normalized_query not in history_search.normalize_for_match(text):
                continue
            if str(leaf.get("coverage_kind") or "") == "legacy_opaque":
                local_legacy_hits.append(str(leaf.get("segment_id") or ""))
            else:
                local_hits.append({
                    "segment_id": leaf.get("segment_id"),
                    "start_seq": leaf.get("start_seq"),
