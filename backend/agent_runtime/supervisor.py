...
class Supervisor:
    ...
    def tick(self, roster: list[dict], *, pre_spawn=None) -> None:
        ...
        tick's new leases are acquired but BEFORE any new consumer is spawned, so
        any batched side-channel setup a consumer needs at startup can be
        configured first (no current caller wires one — the in-CVM LiteLLM
        gateway route this used to configure is retired). It is best-effort — a
        failure logs and the spawns still proceed."""
        ...
        for entry in roster:
            ...
            # Acquired — defer the spawn so any batched side-channel setup runs for
            # the whole batch first (setup-before-spawn; avoids a queued first turn
            # hitting a not-yet-configured side-channel).
            spawned_this_tick += 1
