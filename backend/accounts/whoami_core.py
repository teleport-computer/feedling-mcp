...
def whoami_payload(store: UserStore) -> dict:
    ...
    if tz:
        resp["timezone"] = tz
    # Compatibility field for older settings screens. Legacy stored preferences
    # are retained, but known accounts can no longer opt new content into encryption.
    # Keep enclave public material available for existing ciphertext/local_only.
    resp["content_encryption"] = "off"
    # The write-side field still honors the deployment gate and unknown-user safety.
