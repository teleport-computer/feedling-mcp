...
def model_api_chat_send_core(
    ...
    *,
    ...
    # missing/stale or its host-all flag is off, this turn would park in
    ...
    #
    # For a pi-driven provider (openai_compatible/gemini/openrouter) require the
    # runner to report the pi capability — a runner-pi-off drift would otherwise
    # pass the liveness check yet spawn no consumer, parking the send in
    # "processing". The in-CVM LiteLLM gateway is retired, so there is no
    # gateway gate anymore.
