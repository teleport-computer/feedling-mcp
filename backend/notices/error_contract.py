...
def _chat_specs() -> tuple[ErrorSpec, ...]:
    return (
        ...
        _spec("quota_insufficient", "chat", "provider", "user_provider", "模型服务额度不足，充值后再发消息即可恢复。", en="The model service has insufficient quota. Add credit, then send the message again.", matcher=r"余额|额度|insufficient_quota|insufficient[_ ]?balance|credit balance|requires more credits|payment required|\b402\b|provider_http_402|quota"),
