...
#
# 这是**最后手段**,不是"没填名字"的默认值(2026-08-09 改)。旧判据是
# 「user_name 字段为空 → 一律降级到「对方」」,把"字段没填"当成了"不知道这个人
# 是谁":usr_144b 从没在设置里填过名字(她只跟伴侣说话,不填表),于是 capture
# 读对话写对的「陪她分析母亲的检验报告」,被 dream 按指令追溯改成「陪对方分析
# ……」——一个自称灵魂伴侣的 AI,在自己的记忆里管她叫「对方」。
# 现在的判据是「没有任何可靠的称呼/性别依据」,判断交给看得见身份卡、旧卡和
# 对话的模型(它的证据远多于任何字段检查),这里只兜最后那一档。
...
def _naming_rule(user_name: str) -> str:
    ...
    return (
        ...
        "确实需要主语时：能从身份卡、你和 TA 的关系设定、旧卡或对话材料里"
        "看出本人性别的，就用「他」或「她」——有依据的判断不是猜测，"
        "已经写对的「他」/「她」也保留不动；只有确实看不出来，"
        "才退到中性的「对方」。"
        "任何时候都不要用「用户」/\"user\"、指代本人的「TA」，"
        "也不要用第二人称「你」来指代本人。"
    )
