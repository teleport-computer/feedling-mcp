...
OPERATION_DESCRIPTIONS: dict[Operation, str] = {
    ("post", "/v1/agent/web/search"): (
        ...
        "results. Requires the `web` scope. Authorization is re-checked on every "
    ...
    ("post", "/v1/agent/web/fetch"): (
        ...
        "is independently re-validated (max 5). Requires the `web` scope and the "
        "same three authorization gates as web search, reported in-band as HTTP "
