...
| Hosted Runtime V2 worker pool (planned) | `backend/model_api_runtime/v2/serve_worker.py` is the **decided home** for the V2 bounded worker pool — a sibling entrypoint of the **same backend image**, running **here in the runner CVM** next to the resident consumers + genesis worker (same SKIP-LOCKED-queue precedent), **not** a separate HTTP service and **not** beside the main-app FastAPI backend (see the file's module docstring + spec §2.1). **Not yet started in any manifest** — process startup (a `serve_worker` container/command in this compose, gated by a rollout flag) lands in subproject D's rollout, alongside `hosted_runtime_mode` per-user cutover. This row pins *where* it will run; D wires *that* it runs. |
