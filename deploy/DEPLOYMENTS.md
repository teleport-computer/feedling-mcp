...
| Database | `TEST_DATABASE_URL` is the single primary DSN shared by the main CVM and runner. `TEST_FEEDLING_DATABASE_SCHEMA` defaults to `rds`; Phase 4 promotes the independently owner-migrated TEST TEE Postgres only by switching the DSN and selector together. CI verifies the TEE migration head and application startup contract before either CVM is changed. |
