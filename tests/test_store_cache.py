...
def test_explicit_shell_only_bypass_is_sql_free_in_every_mode(monkeypatch, mode):
    calls = install_counting_loaders(monkeypatch, core_store)
    ...
    assert calls == []
    assert store.loaded_sections() == frozenset()
    assert core_store.get_store_shell_only(
        f"u-shell-{mode}",
...
def test_shell_only_default_preserves_legacy_compatibility(monkeypatch):
    calls = install_counting_loaders(monkeypatch, core_store)
