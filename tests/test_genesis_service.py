...
def test_apply_memory_outputs_preserves_body_for_executor_validation(monkeypatch):
    calls = []
    def fake_execute(_store, _api_key, actions):
        calls.append(actions)
    ...
    monkeypatch.setattr(service.memory_actions, "_execute_memory_actions", fake_execute)
    raw_content = "y" * 5007
    service.apply_memory_outputs(_store(), "api_key", {"memories": [{
        "type": "fact", "summary": "Long card", "content": raw_content,
    }]})
    assert calls[0][0]["memory"]["content"] == raw_content
