...
def test_shared_split_preserves_entire_pre_extraction_daily_summary():
    """Snapshot generated by the pre-T718 build_summary, including every field."""
    expected = json.loads((FIXTURE.parent / "summary_pre_t718.json").read_text())
    assert summary.build_summary(_sources(), day=DAY) == expected
