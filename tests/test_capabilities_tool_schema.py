...
def test_canvas_send_file_requires_bounded_title_and_subtitle():
    send_file = next(
    ...
    assert "requires title and subtitle" in tool_schema.validate_tool_args(
        "send_file", base
    )
    assert tool_schema.validate_tool_args(
        "send_file",
        {**base, "title": "接星星", "subtitle": "六十秒接星星小游戏"},
    ) is None
