...
def test_terminal_rejection_enters_transcript_before_bounded_retry(monkeypatch):
    provider = _ScriptedProvider([
        {
            "reply": "",
            "tool_calls": [{"id": "real-1", "name": "identity_get", "args": {}}],
            "usage": {},
        },
        {
            "reply": "I will keep trying",
            "tool_calls": [{"id": "terminal-real", "name": "identity_get", "args": {}}],
            "usage": {},
        },
        {"reply": "complete answer", "tool_calls": [], "usage": {}},
    ])
    ...
    assert second_messages != third_messages
    rejected = [
        item
    ...
    assert exchange.calls[0].id.startswith(
        tool_loop.REJECTED_TOOL_CALL_ID_PREFIX
    )
    ...
    assert exchange.results[0].metadata == {
        "rejected": "terminal_tool_call_rejected"
    }
    assert "terminal_tool_call_rejected" in exchange.results[0].content
...
def test_malformed_call_gets_fresh_prefixed_paired_rejection_id(monkeypatch):
    provider = _ScriptedProvider([
        ...
        {
            "reply": "trying",
            "tool_calls": [{"id": "", "name": "web_search", "args": {"query": "x"}}],
            "usage": {},
        },
        {"reply": "plain fallback", "tool_calls": [], "usage": {}},
    ])
    ...
    exchange = next(
        item
        for item in provider.calls[2]["messages"]
        if isinstance(item, ToolExchange)
        and any((result.metadata or {}).get("rejected") for result in item.results)
    )
    assert tool_loop.REJECTED_TOOL_CALL_ID_PREFIX == "feedling_rejected_"
    ...
    assert [result.call_id for result in exchange.results] == [
        exchange.calls[0].id
    ]
    transcript_ids = {
        call.id
...
def test_rejected_invented_tool_name_does_not_restore_unknown_schema(monkeypatch):
    provider = _ScriptedProvider([
        {
            "reply": "",
            "tool_calls": [{"id": "invented-real", "name": "invented_tool", "args": {}}],
            "usage": {},
        },
        {"reply": "plain fallback", "tool_calls": [], "usage": {}},
    ])
...
def test_oversized_rejection_transcript_is_bounded_without_tail_copy(monkeypatch):
    ...
    outcome = asyncio.run(tool_loop.run_tool_loop(
        ...
        add_usage=_noop_add_usage,
        max_calls=3,
        max_tool_args_chars=64,
        max_tool_batch_args_chars=128,
    ...
    assert len(json.dumps(
        exchange.calls[0].args,
        ensure_ascii=False,
        sort_keys=True,
        separators=(",", ":"),
    )) <= tool_loop.REJECTED_TOOL_ARGS_SUMMARY_CHAR_CAP
    assert len(exchange.assistant_text) <= (
        tool_loop.REJECTED_ASSISTANT_TEXT_CHAR_CAP
    )
...
def test_rejected_exchanges_do_not_spend_tool_call_budget(monkeypatch):
    provider = _ScriptedProvider([
        {
            "reply": "",
            "tool_calls": [{"id": "valid", "name": "memory_index", "args": {}}],
            "usage": {},
        },
        {
            "reply": "",
        ...
        {
            "reply": "still broken",
            "tool_calls": [{"id": "terminal", "name": "memory_search", "args": {}}],
            "usage": {},
        },
    ])
    monkeypatch.setattr(provider_client, "chat_completion_async", provider)
...
def test_rejected_discovery_key_stays_dispatchable_after_recovery(monkeypatch):
    """A rejected memory-discovery call must never enter the completed-key set.

    Post-rejection rounds are terminal except through file-delivery recovery,
    and recovery rounds only offer delivery tools — so the discovery name must
    already be historical (one real dispatch) for the retried call to pass the
    unknown-tool check. The retried key itself was only ever rejected: if the
    rejection had polluted ``completed_memory_discovery_calls`` the retry would
    be answered by the repeated-discovery stub instead of a real dispatch."""
    provider = _ScriptedProvider([
        {
            "reply": "",
            "tool_calls": [
    ...
    assert [[tc.id for tc in batch] for batch in dispatch.calls] == [["m1"], ["m2"]]
    assert dispatch.calls[1][0].name == "memory_search"
    ...
    # Real dispatcher output, not the repeated-discovery stub text.
    ...
    assert len(rejected_exchanges) == 1
    assert rejected_exchanges[0].results[0].metadata == {
        "rejected": "missing_tool_call_id"
    }
    assert outcome.stop_reason == "required_file_missing"


def test_rejected_round_leaves_stall_counter_continuous(monkeypatch):
    """A rejected batch neither increments nor resets the tool-only stall count.

    Sequence: real tool-only round (count 1) -> rejected tool-only-shaped batch
    (count must stay 1) -> terminal text without the required file (recovery
    disarms the fallback without touching the count) -> two real tool-only
    rounds (count 3 == threshold -> stall). If the rejected round were counted
    the stall would fire on the first of those rounds; if it reset the count the
    stall would not fire before the scripted terminal response."""
    provider = _ScriptedProvider([
        {
            "reply": "",
            "tool_calls": [{
        ...
        {"reply": "post-stall text", "tool_calls": [], "usage": {}},
    ])
    monkeypatch.setattr(provider_client, "chat_completion_async", provider)
    dispatch = _RecordingDispatch()
    surfaces = []
    ...
    outcome = asyncio.run(tool_loop.run_tool_loop(
        ...
        add_usage=_noop_add_usage,
        max_calls=15,
        max_consecutive_tool_only_rounds=3,
        on_provider_tool_surface=record_surface,
    ))

    assert [[tc.id for tc in batch] for batch in dispatch.calls] == [
        ["w1"],
        ["w2"],
        ["w3"],
    ]
    assert [detail["force_text_fallback_reason"] for detail in surfaces] == [
        "none",
        "none",
