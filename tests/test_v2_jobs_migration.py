...
def test_agent_jobs_chain_has_one_installed_head_and_available_at_baseline():
    ...
    assert heads == ["0103_v2_wake_followup_marker"]
    assert (script.get_revision("0103_v2_wake_followup_marker").down_revision
            == "0102_trace_events")
    assert (script.get_revision("0102_trace_events").down_revision
            == "0101_chat_change_events")
    assert (script.get_revision("0101_chat_change_events").down_revision
            == "0100_lane_rollup_access_paths")
    assert (script.get_revision("0100_lane_rollup_access_paths").down_revision
            == "0099_contract_rejection_stats")
    assert (script.get_revision("0099_contract_rejection_stats").down_revision
            == "0098_v1_lane_outcome_counts")
    assert (script.get_revision("0098_v1_lane_outcome_counts").down_revision
            == "0097_v2_job_recovery_events")
    assert (script.get_revision("0097_v2_job_recovery_events").down_revision
            == "0096_trace_write_stats_health")
    assert (script.get_revision("0096_trace_write_stats_health").down_revision
            == "0095_trace_write_stats")
    assert (script.get_revision("0095_trace_write_stats").down_revision
            == "0094_chat_daily_rollup")
    assert (script.get_revision("0094_chat_daily_rollup").down_revision
            == "0093_lane_rollup_voice")
    assert (script.get_revision("0093_lane_rollup_voice").down_revision
            == "0092_lane_rollup_safe_ts")
    assert (script.get_revision("0092_lane_rollup_safe_ts").down_revision
            == "0091_lane_daily_rollup")
    rollup = script.get_revision("0091_lane_daily_rollup")
    assert rollup.down_revision == "0090_merge_wake_outcomes"
...
def test_perception_signal_schema_is_installed_at_the_merged_head():
    ...
    assert installed_head == ("0103_v2_wake_followup_marker",)
