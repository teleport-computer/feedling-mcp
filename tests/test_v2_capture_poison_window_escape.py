...
def test_v2_parse_failures_skip_after_three_on_the_same_frontier():
    ...
    for i in range(1, cs.CAPTURE_POISON_SKIP_AFTER):
        state = _fail(state, error="json_decode_error:JSONDecodeError", window=WINDOW)
...
def test_the_worker_actually_passes_the_window():
    """🔴 调用点真的带上了窗口 —— 不带的话上面这些全是摆设。

    这是这次事故的教训本身：函数写对了、测试也绿，但生产里那条路根本没接上。
    """
    src = (Path(__file__).parent.parent / "backend" / "model_api_runtime" / "v2"
           / "worker.py").read_text("utf-8")
    i = src.index("deps.fail_capture_job,\n")
    call = src[i:i + 600]
    assert "window=dict(capture_window)" in call, (
        "worker 调 fail_capture_job 没带 window —— V2 逃生阀又断了")
