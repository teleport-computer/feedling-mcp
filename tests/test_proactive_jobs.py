...
def test_auto_proactive_v2_schedule_heartbeats_split_presence_and_screen_wakes(tmp_path, monkeypatch):
    ...
    closed_without_frame = client.post(
        "/v1/proactive/tick",
        headers=headers,
        json={"trigger": "broadcast_closed", "broadcast_state": "off"},
    )
    assert closed_without_frame.status_code == 200
    closed_body = closed_without_frame.get_json()
    assert closed_body["enqueued"] is True
    assert closed_body["decision"]["reason"] == "wake_created"
    assert closed_body["decision"]["wake_kind"] == "screen"
    assert closed_body["decision"]["screen_context_available"] is False
    assert closed_body["job"]["wake_kind"] == "screen"
    assert closed_body["job"]["frame_ids"] == []
