...
def test_gateway_provider_derives_pi_unconditionally(client):
    # LiteLLM gateway retired: gemini → pi unconditionally (direct relay, no gateway hop)
    user_id, api_key = _register(client)
    _seed_config(user_id, provider="gemini")
    res = client.post("/v1/model_api/driver", json={}, headers=_headers(api_key))
    assert res.status_code == 200
    # gemini/openrouter/openai_compatible → pi (direct relay)
