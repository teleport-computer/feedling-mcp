...
| Required failure visibility / weak-background silence | `FOREGROUND_NOTICE_WINDOW_SEC` three-bucket fixed window; `usr_6f5a`; 2026-07-27 V2 silent-turn incident; missed scheduled reminders | V2 uses the durable terminal-failure outbox for lanes with a visible delivery obligation. Foreground Chat atomically queues a parent-linked, cursor-fenced encrypted failure bubble plus error status and route-fenced `last_runtime_error`. Scheduled reminders atomically queue a standalone encrypted failure result containing the canonical reminder note and safe attribution; it neither attaches to the latest user turn nor stamps the active route. Heartbeat, manual-wake, screen-watch, Capture, Dream, and maintenance keep their intentional background silence. Both visible forms survive worker crashes and lease expiry without duplication. | ✅ |
...
| Degenerate punctuation fragment drop | `tools/chat_resident_consumer.py:9192-9220,10900-10928,12025-12068`; stream-cut incident `usr_6f5a` emitted punctuation-only bubbles | `worker._is_degenerate_reply` preserves V1's Unicode-category decision. Chat replaces a terminal fragment with the standard fallback and `upstream_unavailable` / `provider_transient` attribution; intermediate fragments are dropped before a later real reply. Heartbeat, manual-wake, and screen-watch fail silently; scheduled failures use the standalone durable failure result required by their delivery contract. | ✅ |
