...
核心：`select_context_memories_with_trace()`（`backend/memgarden/scoring/relevance.py:348`）。
...
`backend/memgarden/scoring/relevance.py:191`，对外封装 `memory_relevance_details()`（`:285`）。分层打分：
...
| 上下文选择主算法 | `backend/memgarden/scoring/relevance.py:348` |
| 相关性评分核心 | `backend/memgarden/scoring/relevance.py:191` |
| 对外封装 `memory_relevance_details` | `backend/memgarden/scoring/relevance.py:285` |
| 通用词表 | `backend/memgarden/scoring/relevance.py:54` / `67` |
