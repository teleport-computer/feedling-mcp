...
38. `✅ T217(2026-08-22) [resident V1 + runtime_v2 Dream 执行链] [梦境整理调度/模型/终态] [已补 memory.dream.start、model.start/done/error、done/error；同一 job_id/trace_id 串联，终态带闭集 outcome 与 proposals/actions/applied/skipped/failed/organized/merged 计数] [部分写入仍按既有幂等语义完成 job，但 trace 必为 status=warning,outcome=partial；全败、映射拒绝、熔断与 provider/parse 失败均为 error，不能再被单个 tick/ok 冒充] [管理端 exact-shape 投影；未知字段整段关闭；已有 V1/V2/HTML+JSON 出口测试，待 test 实弹 readback 后把覆盖矩阵翻绿]`
39. `✅ T217(2026-08-22) [runtime_v2:serve_worker full-card reader + worker._run_extraction] [Dream 输入完整性/最终产物] [完整卡读取明确分 ready/empty/unavailable/truncated；unavailable/truncated 发 memory.extraction.context.error，终态 degraded_context=true] [现在能区分模型在完整/空花园上合法 no-op，与读取失败或预算截断后的降级 no-op] [事件只带 runtime/lane/component/outcome 闭集，不记录 error 文本、卡 id、正文或 prompt；Capture 的上下文降级仍属 A37 后续范围]`
...
| runtime_v2 dream partial memory writes | job 仍按既有幂等语义 mark_completed、tm status=ok；T217 起另发 `memory.dream.done status=warning,outcome=partial` 与逐项计数 | apply_memory_actions 的逐项结果 + Dream terminal trace | 管理端已能看出部分整理/合并没落地；job 状态本身仍不表达 partial | **T217 已消除 trace 假成功；权威 job 状态语义未改** |
