...
**做法**：compaction 生成摘要叶子时，顺带在 enclave 内对该批消息算字符 n-gram
的 Bloom filter 位图（几 KB），加密与摘要同存。查询时在 enclave 内对 query 算
同样的哈希，位图不命中 = **确定性排除**（Bloom 只误报不漏报），命中才解密原文
确认。预期把"500 段筛到 3 段"，解密量下降一到两个数量级。

**泄漏面控制**：位图本身加密存储、只在 enclave 内比对；不得把可比对的盲索引
token 暴露给数据库（否则新增词频/相等性/访问模式泄漏）。

**现在要留的接口**：§4 扫描顺序里"决定优先扫哪段"是一个可替换的过滤器
（今天=摘要匹配，将来=位图匹配），实现上保持这一步与"扫描/解密"解耦，
换过滤器不动其余代码。存量老叶子无位图 → 回落摘要路径，混跑不冲突。
