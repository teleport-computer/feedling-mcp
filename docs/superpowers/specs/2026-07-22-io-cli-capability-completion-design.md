# io_cli 能力补全与通道收口 — 设计(修订版 v3)

- 日期:2026-07-22(v3:吸收 Codex 两轮 plan_review;D2 改为无确认直接执行)
...
| D2(v3 改) | **第一版不做确认,显式请求一律直接执行**(hx 二次拍板,推翻先前确认分级;Codex 两轮建议的确认 challenge 均不采纳,误删/注入残余风险知情接受)。两步式确认凭证挪 §9 后续增强 |
...
- **改名成对(唯一代码硬规则)**:携带 `--agent-name` 的调用必须同时携带
  ...
  profile_patch 入口,但按来源收窄(实现期修正,T1):只拦 agent 来源
  (runtime token);App/用户直连(api_key)不受约束——用户在 UI 单独改名是
  正常路径,无条件闸会破坏"正常路径逐字节不变"(实测误伤 3 个既有用例)。
  agent 的另两条路各自把关:夹带通道在 consumer 漏斗(3.4)执行同规则,
  CLI 走预检**。不做子串猜测(小满/小满满误判从机制上消失)。
...
- **详情**(按需):模型自己跑 `<verb> --help`;规则段(使用规则/例子)
