...
1. **hermes / openclaw 两种自托管 agent 没测**——`user_mcp_materialize.hermes_config_merged`
   只写 url/headers，没有超时旋钮，等不等未知。若也不等，本方案覆盖不到。
2. **补测矩阵未跑**（Codex 建议，接受）：两个受影响模型 × 5 种场景 × 4 次 = 40 个
...
4. **代价未量**（Codex 审出，接受）：需要 A/B 记录用户端总耗时 p50/p95、
