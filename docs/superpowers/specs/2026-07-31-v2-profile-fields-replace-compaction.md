...
**验收**:缓存前缀跨回合**字节一致**;画像 strict-read/decrypt 降级的 `fallback_reason` 进入 turn trajectory/status 面(不能只存在进程日志/计数器);L3 多轮对话在**一个强模型 + 一个弱模型**上各跑一遍(§2-P:强模型会掩盖提示词缺陷),确认模型自然用上画像里的称呼和雷区、且不叙述来源。
