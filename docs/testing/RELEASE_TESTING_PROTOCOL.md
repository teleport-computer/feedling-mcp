...
> **共享克隆里，commit 前先看自己站在哪根分支**——这是同族的第三种，前两条错在"提交
> 内容"和"验证方法"，这条错在**位置**。2026-08-02：我在 Desktop 的 iOS 克隆里提交，
> 落到了 `claude4/vision-capability-notice` 上——那是**别人的本地分支**（远端没有）、
> 上面有 claude4 一笔未推的工作、而且**落后 main 25 个 commit**。`git status` 是干净的，
> 所以什么都没提示我。修法：从 `origin/main` 开新分支 cherry-pick 过去，再
> `git branch -f <他的分支> <他的原 tip>` 复原，**并在新基线上重跑构建**——旧基线上那次
> 不作数（落后 25 个 commit 的树编译通过，不代表在 main 上编译通过）。
> 起手式：`git rev-parse --abbrev-ref HEAD` 和 `git rev-list --count HEAD...origin/main`
> 一起看，别只看 `git status`。
...
| **中转站「API key 未通过测试」（用户报障，2026-08-09）** | 用户换了两个中转站（shanyiapi.com / codeflow.asia）都提示 key 未通过测试，一直在换 key。**实测两站完全健康**：拿一把无效 key 打 `/v1/chat/completions`，两站各自回 401 JSON —— 站活着、协议标准。真因是 **base_url 漏了结尾的 `/v1`**，我们把 URL 问题的报错说成了 key 问题。**分诊手法（30 秒，可复用）**：用一把**明知无效**的 key 去打目标端点 —— 回 **JSON 错误** = 端点正确、只是 key 的事；回 **HTML**（200 首页或 404 页面）= 这个地址根本不是 API 接口。**三个教训**：① 我们的代码**不自动补 `/v1`**（直接拼 `{base_url}/chat/completions`），而 6 个 provider 里 gemini 是 `/v1beta`、bedrock 和 deepseek 无后缀、openrouter 是 `/api/v1` —— **所以也不能无脑补**，只能把错误说清楚；② `base_url` 属敏感字段、admin 面刻意不下发，意味着**改它的行为时你看不到存量影响面**，这本身就是"不该盲改"的理由；③ 后端归因改对了还不够，**客户端会二次翻译**（iOS 把 provider 的 404 映射成"模型不存在"），要一直验到屏幕上那句话。修复见 CHANGELOG 2026-08-09。 |
| **「今天好几次接不上」（用户报障，2026-08-07）** | prod 用户报连续接不上、"以前没怎么遇到"。**结论是我们没坏**：他的模型中转 08-05 上午配额爆掉后开始间歇返回 **HTTP 200 + 空内容**（断流 / 配额紧张时的假成功）。**分诊三步**：①先对齐时间线——把报障窗口和**部署时间**、**用户自己改配置的时间**（`model_api_routes[].updated_at`）并排看，本例 burst 起点与两次部署都对不上、却和他换中转/换模型名的操作精确吻合；②看**舰队面有没有同步变差**（events 页每日回复成功率：部署后反而 92%→98%），只有他一个人坏 = 不是我们的回归；③看他的 `provider_attempt_ledger` 重试次数与 `notice_summaries` 时间线，`attempt≥2` 集中爆发 = 传输层在抽风。**真正的问题在归因不在链路**：中转"规矩地坏"（报错误码）时我们归因正确，"不规矩地坏"（200+空内容）时落进 `reply_parse_failed`/system 兜底，文案说"系统处理回复时出了问题"——把中转抽风包装成 App 故障，用户自然来找我们。修复见 CHANGELOG 2026-08-07（新增 `provider_empty_reply`/provider_transient）。**教训**：①归因模糊时"默认怪自己"是保守但有代价的选择，代价就是用户找错人、真问题拖着不修；②`200 + {"error": insufficient_quota}` 是 one-api/new-api 中转的标准形状，**HTTP 状态码不等于成败**，判空之前先把 body 里的协议层诊断（`error.message`/`finish_reason`）榨出来交给规则表，否则余额为零的用户会被告知"稍后再试、检查中转稳定性"，比原来错得更远。 |
| **记忆花园墓碑卡（用户报障，2026-08-05/06）** | 用户花园出现「已被 \<卡id\> 取代——原文」的墓碑卡（标题正文都是整理注记）。先入为主以为是 dream 结构化 lane——**真凶是 agent 在夜间唤醒里徒手跑 io_cli memory-patch**（明文 `/v1/memory/actions`，绕过全部 parse 层闸）。**分诊判据（先查再猜）**：admin user detail 里 `changes_by_capture_mode` 出现 `unknown` 的 supersede = 徒手 actions 路（≠ dream 结构化路）；新卡 `by_source` 里的 `resident_patch` = memory-patch 徒手写入。**三个反直觉**：①「用户已经自己删了坏卡所以没事」是错觉——删墓碑 ≠ 恢复被它 supersede 吃掉的原卡，内容仍然丢着；②回退方案先跑 recovery dry-run（Actions，只读）拿证据（`hard_deletes` 时间线 / `supersede_by_day` / `plan_rows`）再定窗口，别靠猜——本案 5 次硬删查实是 07-25/27 的旧删除，与墓碑无关；③操作工具打进 **GitHub Actions 日志的报告是泄漏面**：模型可写的自由文本（如 `reason`，≤500 字）可能含卡片内容，只准输出 id/时间戳/枚举 + 存在位（codex2 抓的 P1）。修复：明文路墓碑闸（`memory_card_tombstone`，无条件、拒收在退休旧卡之前）+ recovery `--retire-source` 泛化；永久回归 `test_memory_actions_guard.py`（含 guard-off 三路）、`test_memory_guard_endpoint.py`（真实路由 400 + 旧卡保持 active）。 |
| **定时提醒不能重复（用户报障，2026-08-03）** | 用户要"每天固定时间提醒"，AI 告诉他"只能一天一提醒"。查实：**系统从来没有这个配额**（全仓无 `MAX_SCHEDULED` / `per_day`），`schedule_wake` 只有 `at`/`tz`/`reason`、**没有重复概念**，每次都是一次性闹钟——能力缺失被模型说成了产品限制。修复 `d745acc9`：`repeat` 支持 daily/weekly，固定 +86400/+604800。**这个功能真正的下限不是"能重复"，是"能停"**：用 `repeat_series_id` 串起整串，拿任一 id（含已 fired 的旧 id）即可整串取消；续排在 fire 的 UPDATE 里带 `repeat_canceled_at=0` 条件，取消与触发并发时续排不可能成立。**教训①：用户转述的"限制"先当作模型的说法去查，别当产品规则**——这次和 usr_a40e 的"AI 一直说没接上"同族（那次也是模型在解释一个它并不了解的机制）；**②：给会重复的东西加功能时，"怎么停"要和"怎么开"同时设计**，否则就是造一个关不掉的每日打扰；③ 去重判据必须带上重复种类（`(due_at, repeat)`），否则"每周一 8:00"会被已有的"每天 8:00"静默吞掉。 |
