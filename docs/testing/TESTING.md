...
| **V. 收紧校验 / 加白名单** | 给任何字段加 allowlist、加必填、把「宽松接受」改成「拒绝」 | ✅ | — | — | **必须先拿 prod 真实数据跑一遍**:把线上已存在的取值全集导出，逐个对新规则判定，确认**我们自己代码写的值一个都不会被拒**(2026-07-30 实测:memory source 白名单初版会拒掉 5 个自家值，`resident_absorb` 292 条正是 agent 写记忆的默认 source，上线即让 resident 用户每次写记忆 400)。白名单与**写入端必须同源**(共享常量或 parity 测试)，别两边各写一份。再问一句:**这条新校验会不会把「清理旧脏数据」的路径也一起焊死?**(如 supersede 若继承旧卡的脏字段，就会被自家白名单拒→脏数据永远清不掉) |
