...
### ~~B3-2 沉默需要一等表达~~ → **已由 B4-2 取代,Seven 2026-08-16 已授权**

> ⚠️ **本节作废,不要再依据它停下。** 下面那句「未获授权前不要实现」是
> 2026-08-16 上午写的,当时确实没有授权。**当天下午 Seven 已明确授权**
> (原话:「你和 codex2 一起做完这 7 条就行」,7 条含 #17),
> 且我向他解释过具体机制(加 `stay_silent(reason)` 工具、落库对齐 V1 的
> `wake_result=sleep` + reason)之后他确认 ok。
>
> **实现口径以 B4-2 节为准。** 保留本节仅为历史记录 ——
> 一份规格里同时躺着「授权了」和「没授权」,会让工作者停在一个已经不存在的门前
> (codex2 2026-08-16 就因此升级了一次,那是我文档的错,不是他的错)。
