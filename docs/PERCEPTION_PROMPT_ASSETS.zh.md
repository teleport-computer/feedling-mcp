...
> **2026-08-20 Task 5 已落地**：`backend/perception_kernel/prompts.py` 现在是下表
> 「内核常量」列这些常量的唯一出处，`v2_*` 三行已从各自的 runtime 文件里把这些常量
> 拼回去（golden 7/7 绿）。`v1_reachout_context*` 三行仍是文字判定「归内核」，但常量
> 还没建、`tools/chat_resident_consumer.py` 仍是字面量——那是 Task 6（V1 接线）的范围，
> 见 `docs/superpowers/plans/2026-08-19-perception-extraction-step1.md` Task 6。
