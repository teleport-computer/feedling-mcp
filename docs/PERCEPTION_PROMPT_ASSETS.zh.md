...
> `sensegate.*`**——内核已经从 `backend/perception_kernel/` 搬成外部包
> `sensegate`（装自 https://github.com/teleport-computer/sensegate 的 Release
> wheel，版本钉在 `backend/requirements.lock`；发布前的挂起状态见
> `docs/NOTES-sensegate-swap.md`）。`backend/perception_kernel/` 目录本身已删除，
> 表里出现的所有 `perception_kernel.xxx` 路径只是历史记录，读作
> `sensegate.xxx`（外部包）即可，指代关系不变。
